...
def get_dem_segment(RASTER_DIR, segment_bounds, GRID_XY=True, FLATTEN=False):
    ...
    if NUM_BANDS > 1:
        print(f"Warning: Provided DEM raster has {NUM_BANDS} bands, only the first band will be used for plotting.")
    ...
    raster_current_band = imread(RASTER_DIR, key=0)
    raster[:, :, 0] = raster_current_band[
        Y_MAX_IND:Y_MIN_IND, X_MIN_IND:X_MAX_IND
    ]
...
def get_and_correct_range(data, record_range, SATELLITE):
    ...
    if SATELLITE == "S3":
        ...
        try:
            ...
            HF_IBE_correction = np.zeros_like(
                IBE_correction
            )
        try:
...
def plot(
    ...
    dem_z[dem_z <= -9999] = np.nan  # catch for when nans commonly presented as -9999
